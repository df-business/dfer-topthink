...
abstract class Plugin
{
    ...
    public function __construct()
    {
        ...
        $nameCStyle = dfer_parse_name($this->name);
        ...
        $cmfAdminThemePath    = config('template.dfer_admin_theme_path');
        $cmfAdminDefaultTheme = config('template.dfer_admin_default_theme');
...
//        $app->lang->load($lang_file);
    ...
    final public function getConfig()
    {
        ...
        $pluginCofingKey = 'dfer_' . $name . '_plugin_config';
        if (Cache::has($pluginCofingKey)) {
            return Cache::get($pluginCofingKey);
        }
        $ttl    = mt_rand(600, 6000);
        $config = Db::name('plugin')->cache('dfer_' . $name . '_plugin_config_db', $ttl)->where('name', $name)->value('config');
