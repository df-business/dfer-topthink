...
class AdminInitListener
{
    ...
    public function handle($param)
    {
        ...
        $this->app->lang->load([
            root_path() . "vendor/dfer/topthink/src/lang/{$langSet}.php",
        ]);
        ...
        if (!is_dir($this->app->getAppPath() . $appName) && !is_dir(root_path() . "vendor/dfer/topthink/src/app/{$appName}")) {
            return;
        }

        // 加载核心应用语言包
        $this->app->lang->load([
            root_path() . "vendor/dfer/topthink/src/app/{$appName}/lang/{$langSet}.php",
        ]);
        ...
        foreach ($coreApps as $appName) {
            $langFiles[] = root_path() . "vendor/dfer/topthink/src/app/{$appName}/lang/{$langSet}/admin_menu.php";
        }
        ...
        if (!$plugins->isEmpty()) {
            foreach ($plugins as $plugin) {
                $pluginDir   = dfer_parse_name($plugin['name']);
        ...
        $langFiles[]    = DFER_DATA . "lang/" . $defaultLangDir . "/admin_menu.php";
    ...
    protected function detect(Request $request): string
    {
        ...
        } elseif ($request->cookie('dfer_admin_lang')) {
            // Cookie中设置了语言变量
            $langSet = $request->cookie('dfer_admin_lang');
    ...
    {
        $cookie->set('dfer_admin_lang', $langSet);
    }
}
