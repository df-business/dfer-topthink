...
class PluginRestBaseController extends RestBaseController
{
    ...
    public function getPlugin()
    {
        ...
        if (is_null($this->plugin)) {
            ...
            $pluginName   = dfer_parse_name($pluginName, 1);
            $class        = dfer_get_plugin_class($pluginName);
    ...
    protected function validate($data, $validate, $message = [], $batch = false, $callback = null)
    {
        if (is_array($validate)) {
            ...
            $v = $this->app->validate('\\plugins\\' . dfer_parse_name($this->plugin->getName()) . '\\validate\\' . $validate . 'Validate');
            if (!empty($scene)) {
                $v->scene($scene);
            }
        }
    ...
    public function getRoutePath(): string
    {
        ...
        if (empty($routeRuleName) || $routeRuleName == "plugin/<_plugin>/<_controller?>/<_action?>") {
            $pluginName = $this->request->param('_plugin');
            $pluginName = dfer_parse_name($pluginName, 0);
            $controller = $this->request->param('_controller');
            $controller = dfer_parse_name($controller, 0);
