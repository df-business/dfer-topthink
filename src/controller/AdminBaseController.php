...
class AdminBaseController extends BaseController
{
    ...
    private function updateViewConfig($defaultTheme = '', $viewBase = '')
    {
        ...
        if (empty($defaultTheme)) {
            $cmfAdminDefaultTheme = dfer_get_current_admin_theme();
        } else {
...
//        $path = $themePath . ($app ? $app . DIRECTORY_SEPARATOR : '');
        ...
        if (0 !== strpos($template, '/')) {
            ...
            if ($controller) {
                if ('' == $template) {
                    // 如果模板文件名为空 按照默认规则定位
                    $template = str_replace('.', DIRECTORY_SEPARATOR, $controller) . $depr . dfer_parse_name($request->action(false));
